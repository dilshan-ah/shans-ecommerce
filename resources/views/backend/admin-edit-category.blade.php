                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="border border-3 p-4 rounded">
                            <form action="{{route('admin.update-cat',$category->slug)}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                
                                <div class="mb-3">
                                    <label for="inputProductTitle" class="form-label">Category Title</label>
                                    <input type="text" class="form-control" name="catname" id="inputProductTitle" placeholder="Enter category title" value="{{$category->name}}" required>
                                    @error('catname')
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('catslug') is-invalid @enderror" name="catslug" id="inputProductTitle" value="{{$category->slug}}" placeholder="Enter Category slug" value="{{ old('catslug') }}">
                                    @error('catslug')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="inputProductType" name="status">
                                        ...
                                        <option value="0" @if($category->isactive == 0) selected @endif>Inactive</option>
                                    </select>
                                    @error('status')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
